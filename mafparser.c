...
//Find the next newline in the buffer, if none found, then we need to read in more
//data, moving any leftovers to the beginning of the buffer
...
//Replace newline with a null plug so we can treat whole line as a single string
     *npos=0;
     datum = parser->pos;
//Move parser pos to the first character of the next line
     parser->pos = npos+1;
...
//so change null plug back to new line for next call of this function and reset pos
         if(in_block){
            *npos='\n';
            parser->pos = datum;
            break;
...
void print_sequence_file(FILE *output,seq sequence){
   if(sequence==NULL) return;
   fprintf(output,"s %25s  %18lu  %8u  %c  %18lu  %s\n"
      ,sequence->src,sequence->start,sequence->size
      ,sequence->strand,sequence->srcSize,sequence->sequence);
}
...
void print_alignment_file(FILE *output,alignment_block aln){
   if(aln==NULL)return;
   fprintf(output,"\na %s\n",aln->data);
   for(int i=0; i < aln->size; ++i)
    if(aln->sequences[i] != NULL) print_sequence_file(output,aln->sequences[i]);
}
...
void print_sorted_alignment_file(FILE *output,sorted_alignment_block aln){
   if(aln==NULL) return;
   fprintf(output,"\na %s\n", aln->data);
   int i =0;
   for(;i<aln->in_size; ++i) print_sequence_file(output,aln->in_sequences[i]);
   for(i=0; i < aln->out_size; ++i) print_sequence_file(output,aln->out_sequences[i]);
}
